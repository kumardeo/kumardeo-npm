...
export type ConversationFlavor<C extends Context | undefined = undefined> = {
	...
	? // workaround for https://github.com/microsoft/TypeScript/issues/51111
		...
		? Omit<C, "session"> & LazySessionFlavor<ConversationSessionData & V>
		: C & SessionFlavor<ConversationSessionData>
...
KNOWN_TYPES.set(HttpError.name, {
	...
	create: ([message, error, stack, cause]: any[]) => {
...
class ConversationControls {
	...
	async active() {
		return Object.fromEntries(
			Object.entries((await this[internal].session()).conversation ?? {}).map(
				([id, conversations]) => [id, conversations.length]
			)
		);
	}
		...
		ctx.conversation ??= new ConversationControls(async () => {
			...
			if (!transformed) {
				...
				if (
					...
					session.conversation = delistify(session.conversation, KNOWN_TYPES);
				}
			}
			return session;
		});
...
export class ConversationHandle<C extends Context> {
	...
	async _replayWait(): Promise<C> {
		...
		if (!this._isReplaying) {
			...
			ctx = Object.assign(new Context(u, this.ctx.api, this.ctx.me), x) as C;
	...
	async waitForReplyTo(
		...
		const id =
			typeof message_id === "number" ? message_id : message_id.message_id;
